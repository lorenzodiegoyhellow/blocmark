  ...
  try {
    ...
    if (process.env.NODE_ENV === 'production') {
      const generalLimiter = rateLimit({
        ...
        max: parseInt(process.env.RATE_LIMIT_MAX || '1000'), // Configurable via environment variable
    ...
    if (process.env.NODE_ENV === 'production') {
      const authLimiter = rateLimit({
        windowMs: 15 * 60 * 1000, // 15 minutes
        max: parseInt(process.env.AUTH_RATE_LIMIT_MAX || '100'), // Configurable via environment variable
